      ...
      <div className="glass rounded-3xl p-8 border border-white/50 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start">
          ...
          <form onSubmit={handleUpdateMyProfile} className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
             ...
             <div className="space-y-2">
               <label className="text-xs font-bold text-slate-500 uppercase ml-1">Voz</label>
               <div className="relative">
                 <Music2 className="absolute left-3 top-3.5 text-slate-400" size={18} />
                 <select value={myFormData.voice_part} onChange={(e) => setMyFormData({...myFormData, voice_part: e.target.value})} className="w-full pl-10 px-4 py-3 rounded-xl bg-white/60 border border-slate-200 focus:ring-2 focus:ring-purple-500 outline-none appearance-none cursor-pointer">
             ...
             <div className="space-y-2">
               ...
               <div className="relative">
                 ...
                 <select value={myFormData.group_role} onChange={(e) => setMyFormData({...myFormData, group_role: e.target.value})} className="w-full pl-10 px-4 py-3 rounded-xl bg-white/60 border border-slate-200 focus:ring-2 focus:ring-purple-500 outline-none appearance-none cursor-pointer">
...
// import { useState, useEffect, useRef } from 'react';
// import { supabase } from '../../lib/supabaseClient';
// import { User, Save, Music2, Award, Camera, Loader2, Building2, Calendar, FileText } from 'lucide-react';
// import { compressImage } from '../../lib/imageUtils';
// import { useAppStore } from '../../lib/store'; 
...
//   // --- ESTADOS ---
//   const setGlobalGroupInfo = useAppStore((state) => state.setGroupInfo); 
//   // Nota: Ya no necesitamos profiles, loading general, ni isModalOpen aquí
//   const [isAdmin, setIsAdmin] = useState(false);
//   const [myUserId, setMyUserId] = useState<string | null>(null);
...
//         });
//       }
//       // Nota: Ya no cargamos allProfiles aquí
