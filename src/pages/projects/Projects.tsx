...
// import { useAppStore } from '../../lib/store'; 
// import { Link } from 'react-router-dom';
// import type { Project, Song } from '../../lib/types'; // Asegúrate de importar Song aquí
// import ProjectCard from '../../components/ProjectCard';
// import CreateProjectModal from '../../components/CreateProjectModal';
// import UploadSongModal from '../../components/UploadSongModal'; 
// import { Plus, Music, ChevronRight, Trash2, LayoutGrid, Layers, Loader2 } from 'lucide-react';
...
//   useEffect(() => {
//     const updateTable = async () => {
//       setDisplayedSongs([]); // Limpiar la lista primero
//       setLoadingSongs(true);

//       if (selectedProject === 'ALL') {
//         // Opción A: Mostrar TODO (del store)
//         setDisplayedSongs(allSongs);
//         setLoadingSongs(false);
//       } else {
//         // Opción B: Mostrar solo cantos del proyecto seleccionado
//         try {
//           const { data, error } = await supabase
//             .from('project_songs')
//             .select(`
...
//           // Mapear la respuesta de Supabase a un array limpio de Songs
//           // eslint-disable-next-line @typescript-eslint/no-explicit-any
//           const songsFromProject = data?.map((item: any) => item.songs) || [];
...
//     updateTable();
//   }, [selectedProject, allSongs]); // Se ejecuta si cambia el proyecto o si se actualiza el store global
...
//       // Si borramos el que estaba seleccionado, volver a 'ALL'
//       if (selectedProject !== 'ALL' && selectedProject.id === projectId) {
//         setSelectedProject('ALL');
//       }
//     // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
//                     {/* 1. CARD FIJA "REPERTORIO MAESTRO" (Indestructible) */}
//                     <div 
//                        onClick={() => setSelectedProject('ALL')}
//                        className={`aspect-3/3 rounded-3xl border-2 flex flex-col items-center justify-center gap-2 p-4 cursor-pointer transition-all group
//                          ${selectedProject === 'ALL' 
...
//                 {/* Loader Overlay si cambiamos de proyecto */}
//                 {loadingSongs && (
//                   <div className="absolute inset-0 backdrop-blur-sm z-20 flex items-center justify-center">
//                     <Loader2 className="animate-spin text-[#2dd4bf]" size={32} />
//                   </div>
...
//                         <div className="flex flex-col gap-3">
//                             {displayedSongs.map((song) => (
//                                 <Link key={song.id} to={`/song/${song.id}`} className="bg-white/40 rounded-2xl p-4 hover:bg-white/60 transition-colors group cursor-pointer border border-white/60 shadow-sm">
//                                     <div className="flex items-center justify-between">
//                                         <div className="flex-1 min-w-0">
//                                             <span className="text-base font-bold text-slate-700 hover:text-[#2dd4bf] transition-colors block truncate">
//                                                 {song.title}
...
//                                         </div>
//                                         <div className="flex items-center gap-3 ml-4">
//                                             <span className="bg-slate-100 text-slate-600 px-2.5 py-1 rounded-md text-xs font-bold border border-slate-200 shadow-sm">
//                                                 {song.tone || 'N/A'}
//                                             </span>
//                                             <div className="inline-flex p-2 text-slate-400 hover:text-[#2dd4bf] hover:bg-[#2dd4bf]/10 rounded-full transition-all">
//                                                 <ChevronRight size={20} />
...
//       {/* NOTA: Aquí el modal de subir canto necesitará lógica extra si quieres 
//          que al subir se asigne automáticamente al proyecto seleccionado.
//          Por ahora se queda como 'global'.
//       */}
//       <UploadSongModal 
//         isOpen={isSongModalOpen} 
//         onClose={() => setIsSongModalOpen(false)}
//         projectId="" // Se sube al repertorio maestro por defecto
//         onRefresh={fetchSongs} 
//       />
//     </div>
//   );
